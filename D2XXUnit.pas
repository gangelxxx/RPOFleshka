...
Function ListDevices : FT_Result;
...
function FT_ListDevices(P1: Pointer; P2: Pointer; P3: DWord) : FT_Result ; stdcall ; External FT_DLL_Name name 'FT_ListDevices';
...
Function ListDevices : FT_Result;
var B1,B2,B3: array [0..64] of Char;
    B: array [0..2] of Pointer;
    C,R: DWord;
begin
 ZeroMemory(@B1,SizeOf(B1));
 ZeroMemory(@B2,SizeOf(B1));
 ZeroMemory(@B3,SizeOf(B1));
 ZeroMemory(@B,SizeOf(B));
 B[0]:= @B1;
 B[1]:= @B2;
 B[2]:= nil;
 R:= FT_ListDevices(@B1, @C, FT_LIST_ALL);
 FT_Error_Report('',R);
end;
